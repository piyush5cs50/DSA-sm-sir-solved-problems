...
public class n__queen__branch__and__bound {
    ...
    public static void solve(boolean[][] board, int row, boolean[] columns, boolean[] hdiag, boolean[] vdiag, String afn){
        ...
        for (int col = 0; col < board.length; col++) {
            if(columns[col] == false && hdiag[row + col] == false && vdiag[row - col + board.length] == false){
                columns[col] = true;
                hdiag[row + col] = true;
                vdiag[row - col + board.length] = true;

                solve(board, row + 1, columns, hdiag, vdiag,afn + row + "-" + col + ", ");

                columns[col] = false;
                hdiag[row + col] = false;
                vdiag[row - col + board.length] = false;
            }
        }
